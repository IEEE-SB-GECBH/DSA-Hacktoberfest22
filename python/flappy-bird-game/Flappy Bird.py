...
pygame.display.set_caption("Flapp Bird") # text show on title bar
...
WING_SOUND = pygame.mixer.Sound("sounds/wing.mp3")
...
STAT_FONT = pygame.font.SysFont("FlappyBirdy", 50)
...
def main(genomes, config):
    ...
    while run:
        ...
        for x, bird in enumerate(birds):
            if bird.y + bird.img.get_height() >= 715 or bird.y < 0:
                birds.pop(x)
                nets.pop(x)
                ge.pop(x)
                DIE_SOUND.play()
